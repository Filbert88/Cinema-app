...
function Navbar(){
    ...
    return (
        <div className="w-full h-20 flex items-center justify-between px-20 fixed z-[999px] bg-black">
            <h1 className="text-2xl font-bold">Cinemaxx</h1>
            <button
            ...
            >
                <button className="absolute right-0 top-0 aspect-square xl:hidden" aria-label="Open Menu"
                onClick={() => setIsNavbarExpanded(!isNavbarExpanded)}>
                ...
                <div className="flex flex-col items-center gap-6 justify-center xl:flex-row">
                    {paths.map((path)=> (
                        <Link key={path.name} className="text-xl font-bold" href={path.url}>{path.name}</Link>
                    ))}
                </div>
                <div className="flex flex-col xl:flex-row gap-6 justify-center items-center">
                    <button className="px-5 py-3 rounded-xl bg-black xl:bg-red  font-bold text-xl" onClick={handleLogIn}>Log In</button>
                    <button className="px-5 py-3 rounded-xl bg-black xl:bg-red font-bold text-xl" onClick={handleSignUp}>Sign up</button>
                </div>
            </div>
        </div>
    )
